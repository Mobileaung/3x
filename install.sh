#!/usr/bin/env bash
#
# PGClock Fusion Installer for Pasarguard
# https://github.com/Pasham0/PGClockFusion
#
set -euo pipefail
...
print_banner() {
  log_blank
  hr
  log_line "${C_WHITE}${C_BOLD}  PGClock Fusion Installer for Pasarguard${C_RESET}"
...
prompt_branding() {
  local answer brand_name brand_subtitle brand_logo

  log_blank
  log_line "${C_BOLD}Customize brand? (name, tagline, logo)${C_RESET}"
  ...
  case "${answer,,}" in
    y|yes) ;;
    *)
      export BRAND_NAME="" BRAND_SUBTITLE="" BRAND_LOGO=""
      return 0
      ;;
  esac
...
install_fusion() {
  ...
  info "Installing ${C_BOLD}PGClock Fusion${C_RESET}..."
  prompt_branding

  download_template "$URL_FUSION" "$TARGET_FILE"

  if [[ -n "${BRAND_NAME:-}" || -n "${BRAND_SUBTITLE:-}" || -n "${BRAND_LOGO:-}" ]]; then
...
print_success_box() {
  ...
  log_blank
  log_line "  ${C_BOLD}Template:${C_RESET} PGClock Fusion"
